...
**Last work (2026-07-24) — a run of reported fixes on top of the refinement pass (all shipped to prod, no migration).**
...
| Item                                                             | Note                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Account migration → owner's new Gmail** (planned, not started) | Owner made a fresh **GitHub + Supabase + Vercel** under a new Gmail and wants everything (incl. data) moved there — **reverses D33** (org-owned hosting), consciously. Effort estimated 2026-07-24: **~½ day** if Supabase/GitHub **org-to-org transfer** works (ref/URL/data/auth/Vault/cron stay intact), **~1 day** if recreating with ~no real data, **~1.5–2.5 days** if migrating real `auth.users`. Swing factors: transfer-vs-recreate, and how much real data exists. No edge functions / no storage buckets. Gotchas: 12 env vars; push scheduler reads `push_dispatch_url`/`push_dispatch_secret` from **Supabase Vault** (not in migrations); a URL change cascades to OAuth redirect URIs + Supabase redirect allowlist + `VAPID_SUBJECT` (miss it → reminders die silently); carry VAPID keys over as-is; Google OAuth needs a new client in the new Gmail's Cloud project; Resend/email needs nothing (still domain-blocked). Next step: a step-by-step runbook once the owner confirms transfer-vs-recreate + real-data volume. |
...
| ~~e2e flake: `core-loop.spec.ts:104`~~                           | **Resolved 2026-07-24 — not a bug, and NOT the D45 class I first guessed.** `commitExact` (undo/edit) applies its value straight from `set_count`'s success and never re-seeds from props, so there is no dropped-revalidation path to fix (and making the undo optimistic would reintroduce the count-dip family the code comment warns against). It was the **same worker saturation** as the reflection timeout: under my own back-to-back runs (load >5) the `set_count` round-trip crossed the 5 s assertion window. On a settled machine it passes even at 8 workers; the worker cap gives it margin. Verified 6 consecutive fresh-DB 21/21.                                                                                                                                                                                                                                                                                                                                                                                              |
