...
for t in [
    ...
    ("Personal stretch goals", "a member may raise their OWN daily bar above THEIR share, per task (D51). Raise-only: their share is the floor, so nobody can quietly owe the group less, and an admin raising the target or their share above someone's goal simply wins. It changes only what that member aims at — the ring, the celebration and their reminder. Everything shared or judged (day-completion, streak, consistency, steadfastness, the garden, the circle's collective goal) still counts from their share, so aiming higher can only ever add."),
