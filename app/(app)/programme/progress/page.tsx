 ...
 *
 * ONE STEP IN from the hub at `/programme` (D59), rather than being the hub
 * itself. The two questions an administrator has are "what does the programme
 * ask for" and "who has done it", and this screen answers only the second: with
 * the report AT `/programme`, the content — and the editor on it — was
 * reachable only by noticing that a section heading was a link, which is the
 * thing that made configuring an item feel like a scavenger hunt.
 *
...
export default async function ProgrammeReportPage() {
  ...
  return (
    <Screen>
      {/* Back to the hub, which is now what the Roadmap tab lands on — not to
          /groups. A screen one step in should return to the screen that sent
          it, or the tab and the back link disagree about where you are. */}
      <Link
        href="/programme"
